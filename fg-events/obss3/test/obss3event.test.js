...
assert.strictEqual(record.getRequestParameters().getSourceIPAddress(), "103.218.216.125");
assert.strictEqual(record.getUserIdentity().getPrincipalId(), "9bf43789b1ff4b679040f35cc4f0dc05");

const s3 = record.getS3();
assert.strictEqual(s3.getConfigurationId(), "UK1DGFPYUKUZFHNQ00000160CC0B471D101ED30CE24DF4DB");
...
assert.strictEqual(s3.getBucket().getOwnerIdentity().getPrincipalId(), "0ed1b73473f24134a478962e631651eb");

assert.ok(Array.isArray(event.toJSON().Records));

console.log("obss3-event tests passed");
