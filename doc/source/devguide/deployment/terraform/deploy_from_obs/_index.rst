...
The deployment package for this sample is created with ``npm pack``.
...
- "prepack": This script is executed before the npm pack command.
  it installs all necessary dependencies.
- "postpack": this script is executed after the npm pack command.
  it converts the generated tarball from npm pack to a zip file
  which is needed for deployment to FunctionGraph.
...
- "bundledDependencies" defines which dependencies are bundled in the npm package.

Running ``npm pack`` with this configuration will create a file like ``${npm_package_name}.zip``
in the same folder as the package.json file.
...
- ``zip``: creates the deployment package as zip file using npm pack.
...
- ``test_deployed_sync``: calls the deployed function with a test event and prints the response.
- ``test_deployed_async``: calls the deployed function with a test event in asynchronous way
...
.. list-table:: Backend config variables
  ...
  * - BACKEND_CONFIG_KEY
    ...
    - terraform_state/nodejs/deploy-from-obs.tf
...
.. code-block:: bash
  ...
  # getting the Function URN from terraform output...
  # calling the deployed function via FunctionGraph API...
  {"request_id": "3bc62333-90c3-4357-b37c-ef5404f5de46"}
  # finished
